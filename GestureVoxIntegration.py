...
commands_dict = {
    "commands": {
        ...
        "off": ["switch off", "turn off", "turning off", "turn of", "during of", "during of", "the switch of", "power off"],
        ...
        "SverVse": ["roll down all the windows", "roll down"],
        ...
        "LevelBattery": ["level battery", "battery", "level batery", "batery", "batery level", "battery level"],
        "Music_play_vrem": ["music", "Music"],
        "Pause_play_vrem": ["pause music", "Pause music"],
        "Сontinued_Pause_play_vrem": ["continued ", "Сontinued", "continue ", "Сontinue"],
        "Stop_play_vrem": ["stop music", "Stop music"],
        "Next": ["next music"],
        "Previous": ["previous music", "prev music"],
        "Console": ["Console", "console"],
        "ListenPlayList": ["listen play list", "listen playlist", "playlist", "play list"],
...
commands_dict_openai = {
    "commands": {
        "INPUT_KEY": {"tell me": 0 , "where ": 1, "how much": 2, "when": 3, "how many": 4, "if": 5, "If": 6}
    }
}
...
def delete_files_in_folder(folder_path):
    ...
    if files:
        ...
        for file in files:
            ...
            os.remove(file_path)
            print(f"File {file} successfully delete.")
    else:
        print("No found files in this folder.")
    ...
    if len(mp3_files_test) < len(mp3_files):
        ...
        if mp3_files:
            ...
            for mp3_file in mp3_files:
                ...
                print(f"33333333333333{mp3_file_path}")
                pygame.mixer.music.load(mp3_file_path)
                pygame.mixer.music.play(start=accumulated_time)
                ...
                for filename in os.listdir(directory):
                    ...
                    if os.path.isfile(file_path):
                        ...
                        if ratio >= threshold:
                            ...
                            files_deleted += 1
                        elif ratio > max_ratio and ratio < prev_max_ratio:
                            max_ratio = ratio
...
def window_musik_nuw(nuw):
    ...
    def on_enter_pressed(event=None):
        ...
        if task_text:
            print(task_text)
            if nuw:
                YouTubePlyatlist.search_video_vrem(task_text, r"C:\Users\IVNsell\Desktop\IVNsell\Python\GestureVox Integration\Modern_GUI_PyDracula_PySide6_or_PyQt6-master\HierSpeechpp\HierSpeechpp\playlist_vrem")
                ...
                YouTubePlyatlist.search_video_vrem(task_text, my_path)
                convert_mp4_to_mp3(my_path)
                del_mp4_In_mp3(my_path)
            new_task_entry.delete(0, ctk.END)
    def close_program():
        # Закрытие программы при нажатии на крестик или Escape
        app.destroy()
...
def playsounds(phrase, wait_done=True, ltc = time.time() - 1000):
    ...
    if phrase == "greet":  # for py 3.8
        filename += f"yes_sir_I.wav"
    elif phrase == "ok":
        filename += "done.wav"
    elif phrase == "not_found":
        filename += "What_do_you_want_sir_I.wav"
    elif phrase == "run":
        filename += "good_morning (2).wav"
    elif phrase == "low_battery":
        filename += "low_battery.wav"
        low_battery_say = True
    elif phrase == "off":
        filename += "Power_of_prog.wav"
    #
    if wait_done:
        recorder.stop()
                ...
                def play_audio():
                    ...
                    while True:
                        ...
                        try:
                            ...
                            error_count += 1
                            if error_count >= 10:  # Если возникает ошибка пять раз подряд
                    ...
                    try:
                        if not error_occurred and not speech_finished:
                            streamers = client.chat.completions.create(
                                model="gpt-4-turbo",
                                messages=[{"role": "user", "content": f"{voice}?"}],
                                stream=True,
                            )
                            print(str(voice))
                            Err = False
...
def execute_cmd(k, vab, p):
    ...
    #     playsounds("ok")
        ...
        nuw = True
        threading.Thread(target=window_musik_nuw, args=(nuw,)).start()
    elif k == 'Add_Music_Playlist':
        print("Add new musik to playlist.")
        nuw = False
        threading.Thread(target=window_musik_nuw, args=(nuw,)).start()
    elif k == 'Dell_Playlist':
        print("Delete musik from playlist.")
        threading.Thread(target=window_delete_musik).start()
    elif k == 'ListenPlayList':
        ...
        while waiting_for_response:
            ...
            if os.path.exists(chunk_file):  # Проверяем, существует ли файл перед транскрибацией
                ...
                if fuzz.ratio(transcription, "yes") > 60:
                    print("Yes.")
                    YouTubePlyatlist.search_video_vrem(music, r"C:\Users\IVNsell\Desktop\IVNsell\Python\GestureVox Integration\Modern_GUI_PyDracula_PySide6_or_PyQt6-master\HierSpeechpp\HierSpeechpp\playlist_vrem")
                    threading.Thread(target=play_music_now).start()
    ...
    #     playsounds("ok")
    ...
    elif fuzz.ratio(k, 'Wide_Mode') > 75:#google
        playsounds("ok")
        ...
        if sver_rand == 2:
            pyautogui.hotkey('win', 'down')
        playsounds("ok")
    elif fuzz.ratio(k.join(k.split()[:1]).strip(), 'Volum_0-100') > 75:
        # print(vab)
        ...
        playsounds("ok")
    elif fuzz.ratio(k, 'rob_R') > 60:#google
...
async def main():
    ...
    print(f"Gregory (v5.0) began its work ...")

...
def delayed_action_weather(okey_act):
    ...
    if okey_act:
        if not gesture_performed_weather:
            ...
            webbrowser.open("https://ek.ua/ua/", 1, True)
            time.sleep(0.6)
            gesture_performed_weather = False
...
def delayed_action_anime(Gojo_act):
    ...
    if Gojo_act:
        if not gesture_performed_anime:
            ...
            webbrowser.open("https://animego.org/anime/magicheskaya-bitva-1635", 1, True)
            time.sleep(0.5)
            gesture_performed_anime = False
        ...
        for hand_landmarks in result.multi_hand_landmarks:
            ...
            if mode == "waiting":
                if is_static_hand:
                    ...
                    if index_tip_x > thumb_tip_x and middle_fin_x > index_tip_x and pinky_pip > ring_pip and ring_y > index_tip_y and ring_y > middle_fin_y and middle_ring_pip < 0.05 and middle_ring > 0.05 and ring_x > index_tip_x and pinky_x > index_tip_x:
                        threading.Thread(target=delayed_action_bluetooth, args=(Fix_act,)).start()
                        print("Open bluetooth Settins.")
                    if ((index_tip_x < thumb_tip_x) or (index_tip_y < middle_fin_y)) and index_tip_y < thumb_tip_y and index_tip_y < middle_fin_y and (index_tip_y < pinky_tip_y) and (index_tip_y < ring_y) and ind_mid_fon > 0.001 and index_thump_x > 0.11:#0.1

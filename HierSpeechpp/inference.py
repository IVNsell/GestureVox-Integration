...
def tts(text, a, hierspeech, num):
    ...
    with torch.no_grad():
        w2v_x, pitch = text2w2v.infer_noise_control(token, token_length, src_mel, src_length2,
                                                    noise_scale=a.noise_scale_ttv, denoise_ratio=a.denoise_ratio)
        ...
        converted_audio = \
            net_g.voice_conversion_noise_control(w2v_x, src_length, src_mel, src_length2, pitch,
                                                 noise_scale=a.noise_scale_vc, denoise_ratio=a.denoise_ratio)
...
def inference(a, num):
    ...
    tts(text, a, hierspeech, num)
def main(num, output_path):
    # print('Initializing Inference Process..')
    with open('settings.json', 'r', encoding='utf-8') as json_file:
        data = json.load(json_file)

    name_file_wav = data["Voice asistent"]
    parser = argparse.ArgumentParser()
    parser.add_argument('--input_prompt', default=f'example/{name_file_wav}')
    parser.add_argument('--input_txt', default='example/text.txt')
    parser.add_argument('--output_dir', default=f'{output_path}')
    parser.add_argument('--ckpt', default='./hierspeechpp_eng_kor/hierspeechpp_v1.1_ckpt.pth')
    parser.add_argument('--ckpt_text2w2v', '-ct', help='text2w2v checkpoint path', default='./ttv_libritts_v1/ttv_lt960_ckpt.pth')
    ...
    # print("Path to config.json:", os.path.join(os.path.split(a.ckpt)[0], 'config.json'))
    hps = utils.get_hparams_from_file(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'hierspeechpp_eng_kor\\config.json'))

    hps_t2w2v = utils.get_hparams_from_file(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ttv_libritts_v1\\config.json'))

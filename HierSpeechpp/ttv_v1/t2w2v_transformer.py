...
from ttv_v1 import moduless
from ttv_v1.moduless import WN
...
class ResidualCouplingBlock_Transformer(nn.Module):
    def __init__(self,
        ...
        for i in range(n_flows):
            self.flows.append(
                moduless.ResidualCouplingLayer_Transformer_simple(channels, hidden_channels, kernel_size, dilation_rate,
                                                                 n_layers, gin_channels=gin_channels, mean_only=True,
                                                                 attention_head=4))
            self.flows.append(moduless.Flip())
...
class PosteriorEncoder(nn.Module):
    def __init__(self,
        ...
        self.enc = moduless.WN(hidden_channels, kernel_size, dilation_rate, n_layers, gin_channels=gin_channels)
        self.proj = nn.Conv1d(hidden_channels, out_channels * 2, 1)
...
class StochasticDurationPredictor(nn.Module):
    def __init__(self, in_channels, filter_channels, kernel_size, p_dropout, n_flows=4, gin_channels=0):
        ...
        self.log_flow = moduless.Log()
        self.flows = nn.ModuleList()
        self.flows.append(moduless.ElementwiseAffine(2))
        for i in range(n_flows):
            self.flows.append(moduless.ConvFlow(2, filter_channels, kernel_size, n_layers=3))
            self.flows.append(moduless.Flip())

        self.post_pre = nn.Conv1d(1, filter_channels, 1)
        self.post_proj = nn.Conv1d(filter_channels, filter_channels, 1)
        self.post_convs = moduless.DDSConv(filter_channels, kernel_size, n_layers=3, p_dropout=p_dropout)
        self.post_flows = nn.ModuleList()
        self.post_flows.append(moduless.ElementwiseAffine(2))
        for i in range(4):
            self.post_flows.append(moduless.ConvFlow(2, filter_channels, kernel_size, n_layers=3))
            self.post_flows.append(moduless.Flip())

        self.pre = nn.Conv1d(in_channels, filter_channels, 1)
        self.proj = nn.Conv1d(filter_channels, filter_channels, 1)
        self.convs = moduless.DDSConv(filter_channels, kernel_size, n_layers=3, p_dropout=p_dropout)
        if gin_channels != 0:
...
class PitchPredictor(nn.Module):
    ...
    def forward(self, x, g):
        ...
        for i in range(self.num_upsamples):
            x = F.leaky_relu(x, moduless.LRELU_SLOPE)
            x = self.ups[i](x)
            xs = None
...
class SynthesizerTrn(nn.Module):
  ...
  """
    ...
    def __init__(self,
        ...
        self.enc_p = TextEncoder(178, out_channels=inter_channels, hidden_channels=inter_channels,
                                 filter_channels=inter_channels * 4,
        ...
        self.w2v_decoder = W2VDecoder(inter_channels, inter_channels * 2, 5, 1, 8, output_size=1024, p_dropout=0.1,
                                      gin_channels=256)
    ...
    def forward(self, w2v, length, x_text, text_length, y_mel):
        ...
        return w2v_predicted, (z, z_p, m_q, logs_q), (
        m_p, logs_p), y_mask, pitch_predicted, ids_slice, l_length, phoneme_predicted
    ...
    @torch.no_grad()
    def infer_noise_control(self, x, x_lengths, y_mel, y_length, noise_scale=0.333, noise_scale_w=1, length_scale=1,
                            denoise_ratio=0):
        y_mask = torch.unsqueeze(commons.sequence_mask(y_length, y_mel.size(2)), 1).to(y_mel.dtype)

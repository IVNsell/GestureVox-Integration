...
class ResidualCouplingBlock_Transformer(nn.Module):
    def __init__(self,
        ...
        for i in range(n_flows):
            self.flows.append(
                moduleses.ResidualCouplingLayer_Transformer_simple(channels, hidden_channels, kernel_size, dilation_rate,
                                                                 n_layers, mean_only=True))
            self.flows.append(moduleses.Flip())
...
class PosteriorAudioEncoder(nn.Module):
    def __init__(self,
        ...
        self.enc = moduleses.WN(hidden_channels, kernel_size, dilation_rate, n_layers, gin_channels=gin_channels)
...
class PosteriorSFEncoder(nn.Module):
    def __init__(self,
        ...
        self.source_enc = moduleses.WN(hidden_channels, kernel_size, dilation_rate, n_layers // 2,
                                     gin_channels=gin_channels)
        self.filter_enc = moduleses.WN(hidden_channels, kernel_size, dilation_rate, n_layers // 2,
                                     gin_channels=gin_channels)
        self.enc = moduleses.WN(hidden_channels, kernel_size, dilation_rate, n_layers // 2, gin_channels=gin_channels)
...
class DBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        for layer in self.conv:
            x = F.leaky_relu(x, moduleses.LRELU_SLOPE)
            x = layer(x)
